...
_Важно:_
Делай `git push` только один раз (после 15.50), в течение экзамена ничего пушить не нужно!
...
Ознакомься с файлами в проекте. В папке `src/components` есть стилизованные заготовки многих компонентов, в том числе `Layout.jsx` с подключенными стилями. Использовать их или нет -- решать тебе. Убедись, что в твоем проекте есть .gitignore, установлены все необходимые библиотеки, создай базу данных, подготовь сиды (например, на роли). Используй пакет `dotenv` для хранения учётных данных в переменных окружения (не забудь про `envExample`).
...
_Рисунок 4_. Добавление товара в корзину

### Release 6: Improving Shopping Cart

Настало время усовершенствовать страницу корзины. Сейчас на ней только приветственное сообщение. Сделай так, чтобы на странице корзины отображались все товары, которые пользователь туда добавил. А еще сделай возможным удалять из корзины ненужные товары.

![profile page animation](readme-assets/7.PNG)  
_Рисунок 3_.

### Release 7: Deleting Items

В дальнейшем продавцам понадобится удалить или изменить свои товары. Возможно, они захотят переименовать их, изменить описание или цену.
Добавь кнопки `delete` и `edit` к каждому товару. Реализуй функционал удаления товара, после которого он должен исчезать со страницы с товарами.

![deleting an item](readme-assets/9.PNG)  
_Рисунок 5_. Удаление товара.

_Не забывай, что кнопки **"edit"** и **"delete"** должны быть только у продавцов, при чем только на собственных товарах. Продавцы не могут отредактировать или удалить не свой товар. У покупателей на товарах отображается только кнопка **"add to cart"**._

### Release 8: Updating Items

Добавь в свой проект функциональность, которая позволит пользователям редактировать товар.
Активируй на странице с товарами ссылки `edit` к каждому элементу. По нажатию на нее у продавца должна открываться форма редактирования всех свойств выбранного товара:

![editing an item](readme-assets/8.PNG)  
_Рисунок 6_. Редактирование товара
...
_Рисунок 7_. Просмотр деталей элемента.

### Release 11: Refine

Усовершенствуй работу с сайтом для продавцов и покупателей. Сделай возможным изменять количество товаров в корзине.

**_Примечание:_**
_При изменении количества товара в корзине должно меняться общее количество этого товара на главной странице. Если весь имеющийся товар отправлен в корзину, то он становится недоступным для заказа другим пользователям и пропадает со страницы с товарами._

Убедись, что доступ разграничен правильно. Все ли верно с точки зрения безопасности?
...
**Если вы не успели выполнить задание полностью, в любом случае залейте свой проект на github. Сделать это необходимо после 15.50.**

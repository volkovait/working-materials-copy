...
# Задаём специфический технический вопрос без дополнительного контекста.
# Ожидаем неточный ответ — это исходная точка для сравнения с RAG.
question = "В каком фреймворке url закодированный null является разделителем?"
...
# Загружаем PDF-документ. PyPDFLoader автоматически разбивает PDF на страницы.
file_path = "./data/gotta-cache-em-all.pdf"
loader = PyPDFLoader(file_path)
docs = loader.load()

# Разбиваем страницы на перекрывающиеся чанки.
# add_start_index=True добавляет в метаданные позицию начала чанка в исходном тексте.

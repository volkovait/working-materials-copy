...
from human_interaction import get_human_input, get_human_input_rewind
...
with PostgresSaver.from_conn_string(DB_URI) as checkpointer:
    ...
    config = {"configurable": {"thread_id": "1027"}}

    to_replay = None

    def process_graph_stream(input_data, stream_config):
        """
        ...
        """
        for event in graph.stream(input_data, stream_config):
            for event_name, value in event.items():
                ...
                if isinstance(value, tuple) and hasattr(value[0], "value"):
                    ...
                    action, payload = get_human_input_rewind("Это верно?")
                    print(f"👤 Выбрано действие: {action}")
                    if payload:
                        print(f"✏️ Дополнительный ввод: {payload}")
                    if action == "rewind":
                        return process_graph_stream(None, to_replay.config)
            ...
            states = list(graph.get_state_history(stream_config))
            to_replay = states[-1] if states else None
